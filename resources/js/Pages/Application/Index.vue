<template>
    <app-layout title="Application List">
        <Table>
            <template #headers>
                <tr>
            ...
            <template #data>
                <tr v-for="application in applications.data">
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-gray-500">
                        <vue-countdown :auto-start="!application.hasResolution && application.status !== 'expired' && application.status !== 'disapproved'"
                                       :time="application.application_expiration"
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-gray-500">
                        <Popper arrow content="View Application" :hover="true">
                            <Link :href="route('applications.show',{application:application.id})" class="btn btn-primary p-2">
                        ...
                        <Popper v-if="application.can_update || application.can_update_draft" arrow content="Edit Application" :hover="true">
                            <Link :href="route('applications.edit',{application:application.id})" class="btn btn-success p-2 ml-2">
                                <PencilAltIcon class="h-5 w-5" aria-hidden="true" />
                            </Link>
                        </Popper>
                        <Popper v-if="application.can_extend" arrow content="Request Extension" :hover="true">
                            <Link method="post" :href="route('applications.extension',{application:application.id})" class="btn btn-success p-2 ml-2">
